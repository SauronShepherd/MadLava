...
Filtered JSONL subsets can be exported together with a separate provenance manifest. The provenance manifest records the active type/time filters, source file and line information, correlation evidence, exact UTF-8 byte count, and SHA-256 of the exported JSONL payload. This lets a filtered subset be transferred or archived and later checked against the provenance generated by the offline viewer without changing the authoritative source report.
