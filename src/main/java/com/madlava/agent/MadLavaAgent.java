...
public final class MadLavaAgent {
    ...
    private static void start(String args,Instrumentation instrumentation){try{String version=MadLavaAgent.class.getPackage().getImplementationVersion();if(version==null)version="0.1.0-dev.2";Path output=parseOutput(args);FeatureRegistry registry=new FeatureRegistry();for(String id:new String[]{"heapUsage","nonHeapUsage","bufferPools","garbageCollection","threadStatistics","threadCpu","processResources","classLoaderInsights","jvmExecutionEngine","selfObservability"})registry.register(id,FeatureState.RUNNING);RuntimeContext context=new RuntimeContext(instrumentation,Clock.systemUTC(),output,registry);BoundedSnapshotQueue queue=new BoundedSnapshotQueue(64);JsonlWriter writer=new JsonlWriter(queue,output.resolve("madlava.jsonl"));writer.start();SnapshotScheduler scheduler=new SnapshotScheduler(context,queue,version,sha256(args==null?"":args));scheduler.start(1);Runtime.getRuntime().addShutdownHook(new Thread(()->{scheduler.close();writer.close();},"madlava-shutdown"));System.err.println("MadLava "+version+" ready; output="+output.toAbsolutePath().normalize());}catch(Throwable failure){System.err.println("MadLava bootstrap disabled: "+failure.getClass().getSimpleName());}}
